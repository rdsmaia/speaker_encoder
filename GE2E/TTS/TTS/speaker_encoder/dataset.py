...
class MyDataset(Dataset):
    def __init__(self, ap, meta_data, voice_len=1.6, num_speakers_in_batch=64,
        ...
        self.seq_len_frames = self.seq_len // 220
    ...
    def load_data(self, idx):
        ...
        mel = np.load(wav_file).astype("float32")
#        mel = (mel + 4.) / 8.
        # sample seq_len
...
#                    print(speaker)
                ...
                wav = np.load(utter.replace('/mel-','/mels/mel-'))
#                if wav.shape[0] - self.seq_len > 0:
                if wav.shape[0] - self.seq_len_frames > 0:
                    break
    ...
    def collate_fn(self, batch):
        ...
        for speaker in batch:
            ...
            offsets_ = [random.randint(0, wav.shape[0] - self.seq_len_frames) for wav in wavs_]
#            mels_ = [self.ap.melspectrogram(wavs_[i][offsets_[i]: offsets_[i] + self.seq_len]) for i in range(len(wavs_))]
            mels_ = [wavs_[i][offsets_[i]: offsets_[i] + self.seq_len_frames, :] for i in range(len(wavs_))]
#            feats_ = [torch.FloatTensor(mel) for mel in mels_]
            feats_ = [torch.FloatTensor(mel.T) for mel in mels_]
