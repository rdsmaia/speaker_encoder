...
def build_from_path(hparams, input_dirs, mel_dir, linear_dir, wav_dir, n_jobs=12, tqdm=lambda x: x, prepare_adapt=True):
	"""
	...
	Args:
		...
		- prepare_adapt: whether just spectrograms should be extracted or text input should be input as well
	...
	for input_dir in input_dirs:
		if prepare_adapt:
			...
			with open(os.path.join(input_dir, 'file.list'), encoding='utf-8') as f:
				for line in f:
					basename = line.strip()
					wav_path = os.path.join(input_dir, 'wavs', '{}.wav'.format(basename))
					text = None
