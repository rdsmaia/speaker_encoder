...
from constants import NUM_FBANKS, NUM_FRAMES, EMBEDDING_DIM
...
class DeepSpeakerModel:
    ...
    def __init__(self, batch_input_shape=(None, NUM_FRAMES, NUM_FBANKS, 1), include_softmax=False,
        ...
        # these are magic lines taken from here: https://stackoverflow.com/questions/43147983/could-not-create-cudnn-handle-cudnn-status-internal-error
        # to make DSpeaker able to infer in my small GPU
        # it basically tells TF to not use 100% of the GPU memory
...
#        x = Reshape((-1, 2048))(x)
        ...
        if include_softmax:
            ...
            x = Dropout(0.5)(x)
        x = Dense(EMBEDDING_DIM, name='affine')(x)
        if include_softmax:
