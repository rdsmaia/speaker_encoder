...
from constants import NUM_FRAMES, NUM_FBANKS, EMBEDDING_DIM
...
def sample_from_mfcc_file(utterance_file, max_length):
    mfcc = np.load(utterance_file).astype(np.float32)
...
class KerasFormatConverter:
    ...
    def __init__(self, working_dir, load_test_only=False):
        ...
        if self.categorical_speakers is None:
            print(f'\nSPEAKER IDS: {self.audio.speaker_ids}\n')
    ...
    def generate_per_phase(self, max_length=NUM_FRAMES, num_per_speaker=3000, is_test=False):
        ...
        for i, speaker_id in enumerate(tqdm(self.audio.speaker_ids, desc=desc)):
            ...
            print(f'Number of utterances for speaker {speaker_id} : {len(utterances_files)}')
    ...
    def generate(self, max_length=NUM_FRAMES, counts_per_speaker=(1000, 200)):
...
class LazyTripletBatcher:
    ...
    def update_triplets_history(self):
        ...
        print(f'SPEADER IDS: {speakers}\n')
        print(f'NUMBER OF SPEAKERS: {len(speakers)}')
        selected_speakers = speakers[: self.nb_speakers]
        print(f'NUMBER OF SELECTED SPEAKERS: {len(selected_speakers)}\n')
        embeddings_utterances = []
        for speaker_id in selected_speakers:
            train_utterances = self.sp_to_utt_train[speaker_id]
            #print(f'Number of train utterances for speaker {speaker_id}: {len(train_utterances)}\n')
            #print(f'Speaker id: {speaker_id}, and number of utterances: {len(train_utterances)}')
            #print(train_utterances)
            #for selected_utterance in np.random.choice(a=train_utterances, size=self.nb_per_speaker, replace=False):
            for selected_utterance in np.random.choice(a=train_utterances, size=self.nb_per_speaker, replace=True):
        ...
        embeddings = np.reshape(embeddings, (len(selected_speakers), self.nb_per_speaker, EMBEDDING_DIM))
        self.history_embeddings_train.extend(list(embeddings.reshape((-1, EMBEDDING_DIM))))
    ...
    def get_random_batch(self, batch_size, is_test=False):
        ...
        for anchor_speaker in anchor_speakers:
            ...
            # NOTE: repeats if there is just one sentence
            if len(sp_to_utt[anchor_speaker]) > 1:
                pos_utterances = np.random.choice(sp_to_utt[anchor_speaker], 2, replace=False)
            else:
                pos_utterances = np.random.choice(sp_to_utt[anchor_speaker], 2, replace=True)
        ...
        # NOTE mar 2021: since we are allowing repeat (above), we can set the positive speaker the same as anchor speaker
